...
public class EncrypterPBEWithMD5AndDES {
	private static final int ENCRYPTION_ITERATION_COUNT = 20;
	private static final String ENCRYPTION_TYPE = "PBEWithMD5AndDES";

	public byte[] encryptWithPassword(final byte[] salt,
			final byte[] toEncrypt, final char[] passPhrase)
		...
		try {
			...
			pbeParameterSpecification = new PBEParameterSpec(salt,
	...
	public byte[] decryptWithPassword(final byte[] salt,
			final byte[] toDecrypt, final char[] passPhrase)
			throws EncrypterException {
		try {
			PBEKeySpec pbeKeySpecification;
			PBEParameterSpec pbeParameterSpecification;

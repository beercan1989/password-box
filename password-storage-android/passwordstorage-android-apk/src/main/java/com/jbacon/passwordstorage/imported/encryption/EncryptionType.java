...
import com.jbacon.passwordstorage.imported.encryption.specifications.EncryptionSpecification;
import com.jbacon.passwordstorage.imported.encryption.specifications.EncryptionSpecificationAES;
import com.jbacon.passwordstorage.imported.encryption.specifications.EncryptionSpecificationPBE;
import com.jbacon.passwordstorage.imported.tools.GenericUtils;
...
public enum EncryptionType {
    ...
    public static boolean areValid(final EncryptionType... encryptionTypes) {
        ...
        if (GenericUtils.areNull(encryptionTypes)) { return false; }

        for (final EncryptionType encryptionType : encryptionTypes) {
            if (!isValid(encryptionType)) { return false; }
        }
        return true;
    }
    ...
    public static boolean isValid(final EncryptionType encryptionType) {
        ...
        if (GenericUtils.isNull(encryptionType)) { return false; }

        try {
            return GenericUtils.isNotNull(encryptionType.getEncrypter());
